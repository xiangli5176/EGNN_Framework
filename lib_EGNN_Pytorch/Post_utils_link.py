...
def draw_val_metrics(workdir, tune_param_name, tune_val_label, trainer_id = 0):
    ...
    val_summary = {"Time_train" : Time_train, "Epoch_train" : Epoch_train,  
                    "auc_score" : list(metric_dict["auc_score_hist"].values()), 
                    "ap_score" : list(metric_dict["ap_score_hist"].values()), 
                    }
    ...
    val_optimum = {"auc_score" : max(metric_dict["auc_score_hist"].values()), 
                    "ap_score" : max(metric_dict["ap_score_hist"].values()), 
                    }
...
def generate_val_all_metric(workdir, tune_param_name, tune_val_label_list, tune_val_list, trainer_id_list, real_time = False):
    ...
    for tune_val_label, tune_val in zip(tune_val_label_list, tune_val_list):
        for trainer_id in trainer_id_list:
            ...
            metric_epoch_table = optimum_val_table[["auc_score", "ap_score"]]
            ...
            with pd.plotting.plot_params.use('x_compat', True):
                metric_epoch_table['auc_score'].plot(color = 'g')
                metric_epoch_table['ap_score'].plot(color = 'r')
...
def generate_test_table(workdir, tune_param_name, tune_val_label_list, tune_val_list, trainer_id_list, skip_trainer = None):
    ...
    for tune_val_label, tune_val in zip(tune_val_label_list, tune_val_list):
        for trainer_id in trainer_id_list:
            ...
            with open(test_metric_path, "rb") as fp:
                ...
                auc_score_tune_dict[(tune_val, trainer_id)] = test_metric["auc_score"]
                ap_score_tune_dict[(tune_val, trainer_id)] = test_metric["ap_score"]
    ...
    generate_tuning_raw_data_table(auc_score_tune_dict, test_metric_folder, 
                        file_name="auc_score_tune.csv", tune_param_name = tune_param_name, metric_name = "auc_score", skip_trainer = skip_trainer)
    
    generate_tuning_raw_data_table(ap_score_tune_dict, test_metric_folder, 
                        file_name="ap_score_tune.csv", tune_param_name = tune_param_name, metric_name = "ap_score", skip_trainer = skip_trainer)
...
def plot_raw_tune_test_table(workdir, tune_param_name, tune_val_label_list, tune_val_list, trainer_id_list, skip_trainer = None):
    ...
    file_name_list = ["auc_score_tune.csv", "ap_score_tune.csv"]
    auc_score_raw_df, ap_score_raw_df = [pd.read_csv(os.path.join(test_metric_folder, file_name)) for file_name in file_name_list]    
    
    plt.clf()
    sns.set_style("whitegrid")
    fig, axes = plt.subplots(1, 2, figsize=(16, 8), sharey=False)

    sns.boxplot(ax = axes[0], x=tune_param_name, y="auc_score",  data = auc_score_raw_df)
    sns.boxplot(ax = axes[1], x=tune_param_name, y="ap_score",  data = ap_score_raw_df)
...
def plot_stats_test_table(workdir, tune_param_name, tune_val_label_list, tune_val_list, trainer_id_list, skip_trainer = None):
    ...
    file_name_dict = {"auc_score" : "auc_score_tune.csv", "ap_score" : "ap_score_tune.csv" }
    ...
    mean_metric[tune_param_name] = list(pd.read_csv(os.path.join(stats_metric_folder, file_name_dict["auc_score"]))[tune_param_name])
    ...
    plt.figure()
    with pd.plotting.plot_params.use('x_compat', True):
        mean_stats_df['auc_score'].plot(color = 'r', style='+-')
        mean_stats_df['ap_score'].plot(color = 'g', linestyle='-', marker='o')
    ...
    std_metric = {metric_name : list(pd.read_csv(os.path.join(stats_metric_folder, file_name))["std"]) 
                for metric_name, file_name in file_name_dict.items()}           
    std_metric[tune_param_name] = list(pd.read_csv(os.path.join(stats_metric_folder, file_name_dict["auc_score"]))[tune_param_name])
