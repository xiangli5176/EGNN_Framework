...
from ...evaluation import eva, emb_eva
...
def train(model_train, config, inputs, 
    ...
    """
    feature, labels_full, adj_raw = inputs
    ...
    while epoch <= config["num_epochs"]:
        ...
        for batch_idx, (_, batch_x) in enumerate(train_loader, 1):
            ...
            if (epoch) % config["eval_display"] == 0:
                ...
                kmeans = KMeans(n_clusters = config["n_clusters"]).fit(u)
                predict_labels = kmeans.predict(u)

                metric_res = eva(labels_full, predict_labels, adj_raw)
...
def test(model_template, config, inputs, 
    ...
    test_metric = {}
    test_metric.update(eva(labels_full, predict_labels, adj_raw, emb) )  # only care about Z 
